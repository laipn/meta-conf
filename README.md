...
Incrementally building the data-structure (`IoK8sApiCoreV1Pod`) also makes it obvious the dependency of human-provided-data has with the data-type being created.  Variables declared `with let()` are dependencies of any `@template` or `@data_template`'s called "indented to the right".  `image_name` in this example is a dependency for creating `Container` whereas *prod_env* is a dependency to all aspects of building `IoK8sApiCoreV1Pod`.
